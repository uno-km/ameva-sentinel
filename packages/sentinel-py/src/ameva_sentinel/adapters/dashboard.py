"""
Embedded Security Observability Manager Dashboard Handler for AMEVA-Sentinel Python SDK.
Features OpenStreetMap (OSM) clean tiles, 9 English Continent Viewports, and Country/City Pinpoint Plotting (including Gabon/GA).
"""

from typing import Optional, Any, Dict


def get_sentinel_dashboard_html(
    title: str = "AMEVA-Sentinel Security Observability Manager",
    api_endpoint: str = "/sentinel/events",
    custom_css: str = "",
) -> str:
...
<body>
    <header>
        ...
        <div class="header-controls">
            <span class="tomcat-badge badge-shadow">v2.2.0 Python SDK</span>
        ...
        <div class="status-header-box">
            <div>
                <div class="status-title">
                    ...
                    <span class="tomcat-badge badge-live">PYTHON LIVE SDK</span>
                </div>
                <div style="font-size: 0.85em; color: var(--text-muted); margin-top: 4px;">
                    Endpoint: <code>{api_endpoint}</code> &bull; 
                    ...
                    <tbody id="sessions-tbody">
                        <tr>
                            <td colspan="8" style="text-align:center; color:#64748b; padding:20px;">
                                No recent events. Ready for incoming traffic.
                            </td>
                        </tr>
                    </tbody>
                </table>
...
def mount_fastapi_dashboard(app: Any, path: str = "/sentinel/dashboard", **options: Any) -> None:
    """Mounts the Sentinel Dashboard endpoint into a FastAPI or Starlette application."""
    from starlette.responses import HTMLResponse

    html_content = get_sentinel_dashboard_html(**options)

    @app.get(path, response_class=HTMLResponse, include_in_schema=False)
    async def sentinel_dashboard_view():
        return HTMLResponse(content=html_content, status_code=200)


def mount_flask_dashboard(app: Any, path: str = "/sentinel/dashboard", **options: Any) -> None:
    """Mounts the Sentinel Dashboard endpoint into a Flask application."""
    html_content = get_sentinel_dashboard_html(**options)

    def sentinel_dashboard_view():
        return html_content, 200, {"Content-Type": "text/html; charset=utf-8"}

    app.add_url_rule(path, view_func=sentinel_dashboard_view, methods=["GET"])

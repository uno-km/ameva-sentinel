...
export class HeuristicProfileEngine {
  ...
  static profileSession(footprint: ForensicFootprint): HeuristicVerdict {
    ...
    // 1. Check for Virtual / Headless Cloud Renderers
    ...
    } else if (renderer === 'unknown' || !renderer) {
    ...
    // 2. Check for Software Engineering & Developer Environments
    ...
    if (devFontMatches.length >= 2 && !isCloudHeadless) {
      if (persona !== 'CLOUD_AUTOMATION_BOT') {
    ...
    // 3. Check for Retention & Power Users
    if (visitCount >= 5 && !isCloudHeadless) {
      if (persona === 'DESKTOP_STANDARD') persona = 'POWER_USER';
      tags.push('High_Retention', 'Visits_' + visitCount);
      reasons.push('High engagement session with ' + visitCount + ' accumulated site visits.');
    }

    // 4. Check Power & Hardware Attributes
    ...
    // 5. Generate Natural Language Executive Narrative
